...
export default async function Nav() {
  ...
  return (
    <div className="sticky top-0 inset-x-0 z-50 bg-white shadow-md">
      <header className="relative h-20 mx-auto border-b border-gray-300">
        <nav className="content-container txt-xsmall-plus text-gray-800 flex items-center justify-between w-full h-full text-small-regular">
          <div className="flex-1 basis-0 h-full flex items-center">
            <div className="h-full">
              <SideMenu regions={regions} className="text-gray-800 hover:text-gray-600 transition-colors duration-300" />
            </div>
          </div>
          ...
          <div className="flex items-center h-full">
            <LocalizedClientLink
              href="/"
              className="txt-compact-xlarge-plus hover:opacity-80 uppercase text-gray-900 font-serif font-bold tracking-widest"
              data-testid="nav-store-link"
            >
              Gems Nut
            </LocalizedClientLink>
          ...
          <div className="flex items-center gap-x-8 h-full flex-1 basis-0 justify-end">
            <div className="hidden small:flex items-center gap-x-8 h-full">
              {process.env.NEXT_PUBLIC_FEATURE_SEARCH_ENABLED && (
                <LocalizedClientLink
                  className="bg-yellow-500 text-white font-semibold px-4 py-2 rounded-full shadow-md transition-transform duration-300 transform hover:scale-105 hover:shadow-lg border border-yellow-600"
                  href="/search"
                  scroll={false}
                  data-testid="nav-search-link"
                >
            ...
            >
              <CartButton className="text-gray-900 font-serif font-bold tracking-widest" />
            </Suspense>
          </div>
        </nav>
      </header>
